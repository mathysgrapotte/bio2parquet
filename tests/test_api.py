...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `bio2parquet`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in bio2parquet.__all__ or not hasattr(bio2parquet, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in bio2parquet.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("bio2parquet")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "bio2parquet" or item.name.startswith("bio2parquet."))
        ):
